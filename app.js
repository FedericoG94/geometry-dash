...
function startLevel(i){levelIndex=i;engine=new GameEngine(LEVELS[i]);engine.practice=practice;const id=LEVELS[i].id;save.attempts[id]=(save.attempts[id]||0)+1;saveSave(save);hide(ui.menu);hide(ui.settings);hide(ui.pause);hide(ui.result);hide(ui.editor);controls(true);engine.reset();ui.title.textContent=`${engine.level.name} · ${engine.level.difficulty}`;acc=0;last=performance.now();cancelAnimationFrame(raf);raf=requestAnimationFrame(loop)}
...
function loop(t){if(!engine.running)return;const elapsed=Math.min(.05,Math.max(0,(t-last)/1000));last=t;acc+=elapsed;let steps=0;while(acc>=FIXED&&steps<MAX_STEPS&&engine.running){engine.step(FIXED);acc-=FIXED;steps++}if(!engine.running){finish();return}const p=engine.getProgress();renderer.camera=Math.max(0,engine.players[0].x-renderer.w*.28);ui.progress.style.width=`${p*100}%`;ui.pct.textContent=`${Math.floor(p*100)}%`;render();raf=requestAnimationFrame(loop)}
...
function restart(checkpoint=false){if(checkpoint&&practice&&engine.checkpoint!==null)engine.reset(engine.checkpoint,true);else engine.reset();hide(ui.pause);hide(ui.result);controls(true);last=performance.now();acc=0;raf=requestAnimationFrame(loop)}
...
function inputUp(){engine.release()}
function bindRelease(target){for(const ev of ['pointerup','pointercancel','pointerleave','touchend','touchcancel'])target.addEventListener(ev,inputUp,{passive:true})}
...
window.addEventListener('pointerup',()=>{inputUp();editor?.up()},{passive:true});window.addEventListener('pointercancel',()=>{inputUp();editor?.up()},{passive:true});
ui.touchJump.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();inputDown()},{passive:false});
bindRelease(ui.touchJump);
ui.touchPause.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();pause()},{passive:false});
window.addEventListener('touchend',inputUp,{passive:true});window.addEventListener('touchcancel',inputUp,{passive:true});

window.addEventListener('keydown',e=>{if(editor&&e.key==='Delete'){editor.remove();return}if(['Space','ArrowUp','KeyW'].includes(e.code)){e.preventDefault();inputDown()}if(e.code==='KeyR'){e.preventDefault();restart(false)}if(e.code==='KeyC'&&practice&&engine.running){e.preventDefault();engine.checkpoint()}if(e.code==='Escape'&&engine.running){e.preventDefault();pause()}},{passive:false});
window.addEventListener('keyup',e=>{if(['Space','ArrowUp','KeyW'].includes(e.code))inputUp()},{passive:true});window.addEventListener('blur',inputUp);document.addEventListener('visibilitychange',()=>{if(document.hidden&&engine.running)pause()});
