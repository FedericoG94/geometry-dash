...
const makePlayer=id=>({id,x:100,y:WORLD.FLOOR,size:34,vy:0,gravity:1,mode:MODE.CUBE,onGround:true,alive:true,hold:false,inputBuffer:0,mini:false,rotation:0});
...
export class GameEngine{
  ...
  reset(startX=100,preserve=false){const cp=preserve?this.checkpoint:null,cps=preserve?[...this.checkpoints]:[];this.players=[makePlayer(0)];const p=this.players[0];p.x=Math.max(100,startX);p.y=WORLD.FLOOR;p.vy=0;p.gravity=1;p.mode=MODE.CUBE;p.onGround=true;p.alive=true;p.hold=false;p.inputBuffer=0;p.mini=false;p.size=34;this.speed=clamp(Number(this.level.speed)||285,WORLD.SPEED_MIN,WORLD.SPEED_MAX);this.running=true;this.completed=false;this.crashed=false;this.triggered.clear();this.actions.clear();this.checkpoint=cp;this.checkpoints=cps}
  ...
  processPortals(previousX,currentX){
    for(const o of this.level.objects||[]){if(![OBJECTS.MODE_PORTAL,OBJECTS.GRAVITY_PORTAL,OBJECTS.SPEED_PORTAL,OBJECTS.MINI_PORTAL,OBJECTS.DUAL_PORTAL].includes(o.type))continue;if(!(previousX<o.x&&currentX>=o.x))continue;const k=`${o.type}:${o.x}:${o.y}`;if(this.triggered.has(k))continue;this.triggered.add(k);
      if(o.type===OBJECTS.MODE_PORTAL){for(const p of this.players)this.enterMode(p,o.mode||MODE.CUBE)}
  ...
  enterMode(p,next){
    ...
    p.mode=next;p.vy=0;p.onGround=false;p.inputBuffer=0;p.hold=false;
    p.size=p.mini?24:34;
    if(next===MODE.CUBE||next===MODE.BALL){
      p.y=clamp(p.y,WORLD.FLOOR,WORLD.H-p.size);
      if(p.gravity>0&&p.y<20){p.y=WORLD.FLOOR;p.onGround=true}
      if(p.gravity<0&&p.y>WORLD.H-p.size-20){p.y=WORLD.H-p.size;p.onGround=true}
    }else if(next===MODE.SHIP||next===MODE.UFO||next===MODE.WAVE){
      p.y=clamp(p.y,70,WORLD.H-p.size-70);
    }
    if(previous===MODE.CUBE&&next===MODE.CUBE)p.onGround=true;
  }
  ...
  toggleDual(){if(this.players.length===2){this.players.pop();return}const a=this.players[0],b=makePlayer(1);b.x=a.x;b.mode=a.mode;b.gravity=-a.gravity;b.size=a.size;b.mini=a.mini;b.y=clamp(WORLD.H-a.y-a.size,70,WORLD.H-b.size-70);this.players.push(b)}
